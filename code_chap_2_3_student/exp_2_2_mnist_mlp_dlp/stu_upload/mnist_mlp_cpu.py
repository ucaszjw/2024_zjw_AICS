...
class MNIST_MLP(object):
    ...
    def __init__(self, batch_size=50, input_size=784, hidden1=150, hidden2=130, out_classes=10, lr=0.005, max_epoch=13, print_iter=100):
    ...
    def load_data(self):
        ...
        train_labels = self.load_mnist(os.path.join(MNIST_DIR, TRAIN_LABEL), False) # 读取训练数据的标签
        test_images = self.load_mnist(os.path.join(MNIST_DIR, TEST_DATA), True) # 读取测试数据的图像
        test_labels = self.load_mnist(os.path.join(MNIST_DIR, TEST_LABEL), False) # 读取测试数据的标签
    ...
    def build_model(self):  # 建立网络结构
        ...
        self.fc2 = FullyConnectedLayer(self.hidden1, self.hidden2) # 建立全连接层2
        self.relu2 = ReLULayer() # 建立 ReLU 层2
    ...
    def forward(self, input):  # 神经网络的前向传播
        ...
        h2 = self.fc2.forward(h1) # 前向传播计算全连接层2的输出
        h2 = self.relu2.forward(h2) # 前向传播计算 ReLU 层2的输出
        h3 = self.fc3.forward(h2) # 前向传播计算全连接层3的输出. 然后将全连接层3的输出作为 softmax 层的输入
    ...
    def backward(self):   # 神经网络的反向传播
        ...
        dh3 = self.fc3.backward(dloss) # 反向传播计算全连接层3的损失
        dh2 = self.relu2.backward(dh3) # 反向传播计算 ReLU 层2的损失
        dh2 = self.fc2.backward(dh2) # 反向传播计算全连接层2的损失. 然后将全连接层2的损失作为 ReLU 层1的损失
    ...
    def train(self):
        ...
        print('Epoch number: %d, Batch size: %d, Iteration number per epoch: %d' % (self.max_epoch, self.batch_size, max_batch))
