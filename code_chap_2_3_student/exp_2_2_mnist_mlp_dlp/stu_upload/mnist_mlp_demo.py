...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784, 
        ...
        print('Building MNIST MLP model with input %d, hidden1 %d, hidden2 %d, output %d.' % (input_size, hidden1, hidden2, out_classes))
        ...
        # input_shapem1 = [batch_size, 1, 1, input_size]
        ...
        axis = 1
        self.net.createSoftmaxLayer('softmax', input_shapem4, axis)
    ...
    def load_data(self, data_path, label_path):
        ...
        test_images = self.load_mnist(data_path, True) # 读取测试数据的图像
        test_labels = self.load_mnist(label_path, False) # 读取测试数据的标签
