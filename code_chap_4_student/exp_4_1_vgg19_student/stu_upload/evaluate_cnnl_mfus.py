...
def vgg19():
    ...
    for i, layer_name in enumerate(layers):
        if layer_name.startswith('conv'):
            ...
            layer_container.add_module(layer_name, conv)
        elif layer_name.startswith('relu'):
            # TODO: 在时序容器中执行ReLU计算
            relu = nn.ReLU(inplace=True)
            layer_container.add_module(layer_name, relu)
        elif layer_name.startswith('pool'):
            # TODO: 在时序容器中执行maxpool计算
            pool = nn.MaxPool2d(kernel_size=2, stride=2)
            layer_container.add_module(layer_name, pool)
        elif layer_name == 'flatten':
            # TODO: 在时序容器中执行flatten计算
            flatten = nn.Flatten()
            layer_container.add_module(layer_name, flatten)
        elif layer_name == 'fc6':
            # TODO: 在时序容器中执行全连接层计算
            in_features = 512 * 7 * 7
            out_features = 4096
            fc6 = nn.Linear(in_features, out_features)
            layer_container.add_module(layer_name, fc6)
        elif layer_name == 'fc7':
            # TODO: 在时序容器中执行全连接层计算
            in_features = 4096
            out_features = 4096
            fc7 = nn.Linear(in_features, out_features)
            layer_container.add_module(layer_name, fc7)
        elif layer_name == 'fc8':
            # TODO: 在时序容器中执行全连接层计算
            in_features = 4096
            out_features = num_classes
            fc8 = nn.Linear(in_features, out_features)
            layer_container.add_module(layer_name, fc8)
