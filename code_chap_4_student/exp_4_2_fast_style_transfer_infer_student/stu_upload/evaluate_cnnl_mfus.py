...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        #TODO: 返回残差运算的结果
        return nn.ReLU(inplace=True)(nn.Identity()(x) + self.layer(x))
...
if __name__ == '__main__':
    ...
    for i, image in enumerate(data_group):
        ...
        input_image_c = image_c.to(torch.device('mlu'))
        #将net_trace模型拷贝到MLU设备，得到net_mlu
        net_mlu = net_trace.to(torch.device('mlu'))
        start = time.time()
        ...
        save_image(torch.cat([image_g_mlu, image_c], dim=3), f'./out/mlu_cnnl_mfus/{i}.jpg')
