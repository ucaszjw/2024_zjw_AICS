...
class VGG19(nn.Module):
    def __init__(self):
        ...
        self.layer1 = torch.nn.Sequential(*list(a.children())[:2])
        #TODO: 定义self.layer2为第4层卷积后对应的特征
        self.layer2 = torch.nn.Sequential(*list(a.children())[2:4])
        #TODO: 定义self.layer3为第8层卷积后对应的特征
        self.layer3 = torch.nn.Sequential(*list(a.children())[4:8])
        #TODO: 定义self.layer4为第12层卷积后对应的特征
        self.layer4 = torch.nn.Sequential(*list(a.children())[8:12])
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        #TODO: 返回残差运算的结果
        return nn.ReLU()(nn.Identity()(x) + self.layer(x))
...
if __name__ == '__main__':
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            image_g = g_net.forward(image_c)
            #TODO: 利用特征提取网络提取生成图像的特征out1、out2、out3、out4
            out1, out2, out3, out4 = net.forward(image_g)
            ...
            c1, c2, c3, c4 = net.forward(image_c)
            ...
            if i % 10 == 0:
                ...
                save_image(torch.cat((image_g, image_c), 3), './out/train/%d-%d.jpg' % (j, i))
        j += 1
